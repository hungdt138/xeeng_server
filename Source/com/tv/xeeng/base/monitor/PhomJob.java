 ...
 * @author tuanda
 */
public class PhomJob implements Job {
    private static Logger log = LoggerContext.getLoggerFactory().getLogger(PhomJob.class);
    ...
    protected void doTimeout(Vector<RoomEntity> rooms)
    {
        ...
        for (int i = 0; i < rooms.size(); i++) {
            ...
            {
                log.error("do timeout phom error matchId" + matchId, ex);
            }  
        }
    }
    ...
    public void execute(JobExecutionContext jec) throws JobExecutionException {
        if(!isInProgress)
        {
            ...
            try {
               Zone zone = zManager.findZone(ZoneID.PHOM);
               Vector<RoomEntity> rooms = zone.dumpPlayingRooms(ZoneID.PHOM);
                doTimeout(rooms);
